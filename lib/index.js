...
const fetchAPI = () => {
  fetch(`https://api.openweathermap.org/data/2.5/weather?q=${newLocation.value}&units=metric&appid=7bf029f80cf5f7c9d41a9a29585d592e`)
...
const fetchWeatherByCoordinates = navigator.geolocation.getCurrentPosition((position) => {
  ...
  fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&units=metric&appid=7bf029f80cf5f7c9d41a9a29585d592e`)
    .then(response => response.json())
    .then((data) => {
      console.log(data);
      const timezone = (data.timezone);
